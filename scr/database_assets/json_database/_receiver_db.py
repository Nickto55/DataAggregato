...
class ReceiverDataBase:
    def __init__(self, name_file_database=None, data_structur=None, additional_directories=None, config_programm=None):
        """
        Обработчик базы данных
        """
        self.name_programm_config_dir = ".Aggregato"
        self.name_work_dir = 'configs'
        self.name_work_file = name_file_database if not name_file_database is None else 'database.json'

        self.confifig_program = config_programm

        self.CONFIG_DIR = os.path.join(os.path.expanduser("~"),
                                       os.path.join(self.name_work_dir, self.name_programm_config_dir))
        if not pd.isna(additional_directories):
            self.CONFIG_DIR = os.path.join(self.CONFIG_DIR, additional_directories)
        self.file_path = os.path.join(self.CONFIG_DIR, self.name_work_file)

        if not pd.isna(self.confifig_program):
            self.confifig_program.set_config_progrm(key='path to directory database save reports', data=self.CONFIG_DIR)

        self.data = data_structur
        self.data_structur = data_structur if not pd.isna(data_structur) else {}
    ...
    def load(self):
        ...
        try:
            ...
            print("база данных ауста, загрузка содержимого не удалась")
    ...
    def recovery_lose_key_and_value(self):
        ...
        self.data, was_changed = _merge_missing(self.data, self.data_structur.copy())
