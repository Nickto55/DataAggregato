...
class ExcelDataInserter:
    ...
    def insert_data(self, data, sheet_name, headers=None):
        ...
        for col_idx, header in enumerate(headers, 1):
            ...
            if header == '  back  ':
                cell.fill = PatternFill(start_color="f15263", fill_type="solid")
                cell.font = Font(color="f2ecde")
                cell.hyperlink = f"#'{'All programs'}'!A1"
                continue
        ...
        for product_key, product_value in data.items():
            for detail_key, detail_value in product_value.items():
                for col_idx, header in enumerate(headers, 1):
                    cell = ws.cell(row=row_idx, column=col_idx, value=detail_value.get(header, ''))
                    if header == 'version':
                        cell.hyperlink = detail_value.get(header, '')
                        cell.value = os.path.basename(detail_value.get(header, ''))
                        cell.font = Font(color='266a99')
                    if header == 'Название программы':
                        cell.hyperlink = f"#'{detail_value.get(header, '')}'!A1"
                        cell.font = Font(color='266a99')
